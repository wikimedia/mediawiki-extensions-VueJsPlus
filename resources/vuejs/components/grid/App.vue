...
const ColCell = require( './ColCell.vue' );
const DataRow = require( './DataRow.vue' );
...
module.exports = exports = {
	...
	data: function () {
		...
		for ( let index = 0; index < this.cols.length; index++ ) {
			if ( this.cols[ index ].hasOwnProperty( 'dataIndex' ) ) {
				const dataIndex = this.cols[ index ].dataIndex;
	...
	methods: {
		doUpdateModel: function ( option ) {
			const dataIndex = option.dataIndex;
			console.log( option );
			...
			if ( option.name === 'SortASC' ) {
				...
				this.datacols.find( ( obj ) => {
					return obj.dataIndex === dataIndex;
				} ).ariaSorted = 'ascending';
			...
			function sortHtmlASC( rows, dataIndex ) {
				...
				function sortFunction( a, b ) {
					...
					const aText = $( '<div>' + a + '</div>' ).text();
					b = b.data.find( ( obj ) => {
						return obj.dataIndex === dataIndex;
					} ).content.toLowerCase();
					const bText = $( '<div>' + b + '</div>' ).text();
			...
			function getDateTimestamp( text ) {
				...
				const isEuro = text.match( /^\d{1,2}\.\d{1,2}\.\d{2,4}$/ );
				let isIso = text.match( /^\d{2,4}-\d{1,2}-\d{1,2}$/ );
				if ( isEuro ) {
					text = text.split( '.' );
			...
			function filterStringSingleColumn( rows, dataIndex, filterString ) {
				...
				if ( filterString === '' ) {
					...
					for ( var index = 0; index < rows.length; index++ ) {
						const content = rows[ index ].data.find( ( obj ) => {
							return obj.dataIndex === dataIndex;
						} ).content.toLowerCase();
			...
			function filterHTMLSingleColumn( rows, dataIndex, filterString ) {
				...
				if ( filterString === '' ) {
					...
					for ( var index = 0; index < rows.length; index++ ) {
						...
						const matches = [ ...content.matchAll( '<img.*?alt="(.*?)".*?>' ) ];
						if ( matches.length > 0 ) {
			...
			function filterDateSingleColum( rows, dataIndex, filter ) {
				if ( filter.from === 0 && filter.to === 0 ) {
					...
					for ( var index = 0; index < rows.length; index++ ) {
						...
						const timestamp = getDateTimestamp( content );
			...
			if ( data.hasOwnProperty( 'callback' ) ) {
				...
				if ( this.selectable === true ) {
					...
					for ( let index = 0; index < this.rows.length; index++ ) {
						const callbackRow = this.rows[ index ];
...
var prepareRows = function ( rows, availableRows, dataIndexColumnTypeMap, isSelectable, isSelected, renderer ) {
	for ( let index = 0; index < rows.length; index++ ) {
		...
		for ( let idx = 0; idx < row.data.length; idx++ ) {
